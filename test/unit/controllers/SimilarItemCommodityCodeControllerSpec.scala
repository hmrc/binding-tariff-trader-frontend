...
class SimilarItemCommodityCodeControllerSpec extends ControllerSpecBase {
  ...
  "SimilarItemCommodityCode Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(SimilarItemCommodityCodePage.toString -> JsString(SimilarItemCommodityCode.values.head.toString))
      val getRelevantData = new FakeDataRetrievalAction(Some(CacheMap(cacheMapId, validData)))

      val result = controller(getRelevantData).onPageLoad(NormalMode)(fakeRequest)

      contentAsString(result) mustBe viewAsString(form.fill(SimilarItemCommodityCode.values.head))
    }

    "redirect to the next page when valid data is submitted" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", SimilarItemCommodityCode.options.head.value))
