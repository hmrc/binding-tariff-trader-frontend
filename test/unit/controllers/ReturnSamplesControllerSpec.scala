...
class ReturnSamplesControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new ReturnSamplesController(FakeDataCacheConnector, new FakeNavigator(onwardRoute), FakeIdentifierAction(messagesControllerComponents),
  ...
  "ReturnSamples Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustBe viewAsString(form.fill(ReturnSamples.values.head))
    }
