...
class ConfirmationUrlViewModelSpec extends UnitSpec {
  ...
  "confirmationUrlViewModel" should {
    "create a ConfirmationBTAUrlViewModel" when {
      "there is a value for the btaHomeUrl" in {
        val btaReturnPath = "/testBTAUrl"
        val expectedUrl = s"$btaHostUrl$btaReturnPath"
        val expectedViewModel: ConfirmationBTAUrlViewModel = ConfirmationBTAUrlViewModel(expectedUrl)

        ConfirmationUrlViewModel(btaHostUrl, Some(btaReturnPath)) shouldBe expectedViewModel
        expectedViewModel.messageKey shouldBe "view.bta.home.link"
      }
    }

    "create a ConfirmationHomeUrlViewModel" when {
      "there is no value for btaHomeUrl" in {
        val expectedViewModel: ConfirmationHomeUrlViewModel.type = ConfirmationHomeUrlViewModel

        ConfirmationUrlViewModel(btaHostUrl, None) shouldBe expectedViewModel
