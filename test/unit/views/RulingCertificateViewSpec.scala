...
class RulingCertificateViewSpec extends ViewSpecBase {
  ...
  private def createPdfView(c: Case): HtmlFormat.Appendable = {
    rulingCertificateTemplate(frontendAppConfig, c, c.decision.get)(messages, fakeRequest)
  }

  private def createHtmlView(c: Case): HtmlFormat.Appendable = {
    rulingCertificateTemplate(frontendAppConfig, c, c.decision.get, compositeMode = true)(messages, fakeRequest)
  }
