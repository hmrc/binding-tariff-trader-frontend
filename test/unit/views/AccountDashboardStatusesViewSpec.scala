...
class AccountDashboardStatusesViewSpec extends ViewBehaviours with PaginationViewBehaviours {
  ...
  def applicationView(dashboard: Dashboard, btaUrl: Option[String] = None): () => HtmlFormat.Appendable =
    () => accountDashboardStatusesView(frontendAppConfig, dashboard, btaUrl)(fakeRequest, messages)
  ...
  "Applications and ruling view" must {
    ...
    "display a return to BTA link" when {
      "a user has arrived from BTA" in {
        val btaPath = "testBTAReturnPath"
        val expectedBtaUrl = s"$btaHost$btaPath"
        val doc = asDocument(applicationView(Dashboard(emptyPaged, Sort()), Some(btaPath))())
        assertLinkContainsHref(doc, "bta-return-link", expectedBtaUrl)
      }
    }
