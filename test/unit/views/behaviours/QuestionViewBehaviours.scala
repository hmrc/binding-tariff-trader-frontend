...
trait QuestionViewBehaviours[A] extends ViewBehaviours {
  ...
  protected val error = FormError(errorKey, errorMessage)

  protected val form: Form[A]

  protected def pageWithTextFields(createView: Form[A] => HtmlFormat.Appendable,
      ...
      for(field <- fields) {
        s"rendered with an error with field '$field'" must {
          ...
          s"show an error in the label for field '$field'" in {
            ...
            errorSpan.parent.getElementsByTag("label").attr("for") mustBe field
          }
        }
      }
