...
class SelectApplicationTypeViewSpec extends ViewBehaviours {
  ...
  "SelectApplicationType view" when {
    "rendered" must {
      "contain radio buttons for the value" in {
        val doc = asDocument(createViewUsingForm(form))
        for (option <- SelectApplicationType.options) {
    ...
    for(option <- SelectApplicationType.options) {
      s"rendered with a value of '${option.value}'" must {
        s"have the '${option.value}' radio button selected" in {
          ...
          for(unselectedOption <- SelectApplicationType.options.filterNot(o => o == option)) {
