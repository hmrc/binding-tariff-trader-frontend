...
object ConfirmationUrlViewModel {
  def apply(btaHostUrl: String, btaHomePath: Option[String]): ConfirmationUrlViewModel = {
    btaHomePath match {
      case Some(btaPath) => ConfirmationBTAUrlViewModel(s"$btaHostUrl$btaPath")
      case None => ConfirmationHomeUrlViewModel
    }
  }
}
